...
def main(img_path, t_select, c_select, img_colour="Greens", render_colour="green", show_figures=True,
         render=False):
    ...
    plot_img(img=img_raw, scale=img_scale, unit=img_unit, x_i=x_i, y_i=y_i, z_i=z_i,
             savefig=os.path.join(resfig_dir, "sliced_raw.pdf"), cmap=f"{img_colour}_r", hidefig=hidefig)
    plot_img(img=img_raw, scale=img_scale, unit=img_unit, max_proj=True, cmap=img_colour,
